...
let () =
  ...
  let cd =
    try Helpers.ethereum_cd_of_hex (Array.get Sys.argv 2) with
    | Invalid_argument _ -> [||]
  in
  ...
  let count = ref 0 in
  let sim = ref (Cpu.make ~e_cd:cd ~b:mem ~r:registers ~pc ()) in
  let cycles = ref Cycle_detection.empty in
  let last_ops = ref [] in
  let print_cleanup _ =
    Cpu.pp fmt_stderr !sim;
    (* TODO make cleaner *)
    Format.fprintf fmt_stderr "@.CYCLE DETECTION: %a" Cycle_detection.pp !cycles;
    Format.fprintf fmt_stderr "@.REGISTERS CIRCULAR BUFFER: %a"
      Circ_registers.pp circ_registers;
    Format.fprintf fmt_stderr "@.WORDS CIRCULAR BUFFER: %a" Circ_hex.pp
      circ_words;
    Format.fprintf fmt_stderr "@.LIFTED CIRCULAR BUFFER: %a" Circ_lifted.pp
      circ_lifted;
    Format.fprintf fmt_stderr "@.PC CIRCULAR BUFFER: %a" Circ_hex.pp circ_pc;
    Format.fprintf fmt_stderr
      "@.CURRENT STACK POINTER: %lx@.INSTRUCTION COUNT: %l" !sim.pc !count;
    Format.fprintf fmt_stderr "@.UNIQUE OP NAMES SEEN: %a@."
      (Format.pp_print_list Lifted.pp)
      !last_ops
  in
  ...
  try
    while true do
      ...
      sim :=
        Simulator.step
          ~before_lift:(fun x ->
            Circ_hex.push circ_words x;
            x)
          ~after_lift:(fun x ->
            Circ_lifted.push circ_lifted x;
            x)
          ~fmt:fmt_stderr !sim;
      match last_op with
      | Some op ->
